...
use crate::rpc_service_factory::ForkNodeRpcServiceFactory;
use crate::ForkNodeHandle;
use crate::remote_state::RemoteStateView;
// use crate::chain_service::ForkChainReaderService;
use anyhow::anyhow;
use actix::prelude::*;
...
impl ServiceFactory<Self> for ForkNodeService {
    fn create(ctx: &mut ServiceContext<ForkNodeService>) -> Result<ForkNodeService> {
        Ok(Self {
...
impl ActorService for ForkNodeService {}

impl ServiceHandler<Self, NodeRequest> for ForkNodeService {
    fn handle(
        &mut self,
        msg: NodeRequest,
        ctx: &mut ServiceContext<ForkNodeService>,
    ) -> Result<NodeResponse> {
        Ok(match msg {
...
impl ForkNodeService {
    pub fn launch(
        rpc: String,
        fork_number: Option<u64>,
        config: Arc<NodeConfig>,
        logger_handle: Arc<LoggerHandle>,
    ) -> Result<ForkNodeHandle, NodeStartError> {
        ...
        let join_handle = timeout_join_handler::spawn(move || {
            ...
            system.block_on(async {
                match Self::init_system(rpc, fork_number, config, logger_handle).await {
                    Err(e) => {
        ...
        Ok(ForkNodeHandle::new(join_handle, node_service, registry))
    }
    ...
    async fn init_system(
        ...
        config: Arc<NodeConfig>,
        logger_handle: Arc<LoggerHandle>,
    ) -> Result<(ServiceRef<RegistryService>, ServiceRef<ForkNodeService>)> {
        let registry = RegistryService::launch();

        let remote_client = Arc::new(
            RemoteStateView::from_url(&rpc[..], fork_number)?
        );
        registry.put_shared(config.clone()).await?;
        registry.put_shared(logger_handle).await?;
        registry.put_shared(remote_client).await?;
        ...
        let node_service = registry.register::<ForkNodeService>().await?;
        registry
            .register_by_factory::<RpcService, ForkNodeRpcServiceFactory>()
            .await?;
